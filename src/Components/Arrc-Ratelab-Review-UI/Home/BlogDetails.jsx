  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        const postResponse = await axios.get(`http://127.0.0.1:5000/api/posts/${id}`);
        setPost(postResponse.data);

        // Fetch the latest blog posts
        const latestResponse = await axios.get("http://127.0.0.1:5000/api/posts");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white shadow-md rounded-md overflow-hidden">
            ...
            <div className="p-6">
              ...
              <p className="text-gray-500 text-sm mt-2">{new Date(post.createdAt).toLocaleDateString()}</p>
              <div
          ...
          <div className="bg-white shadow-md rounded-md p-4">
            ...
            <ul className="space-y-4">
              {latestPosts.map((latestPost) => (
                <li key={latestPost._id} className="flex items-center">
                  <div className="w-16 h-16 bg-gray-200 rounded-md overflow-hidden flex-shrink-0">
                    <img
                      src={latestPost.featureImage}
                      alt="Thumbnail"
                      className="w-full h-full object-cover"
                    />
                  </div>
