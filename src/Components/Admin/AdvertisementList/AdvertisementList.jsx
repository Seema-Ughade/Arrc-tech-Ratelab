...
const AdvertisementList = () => {
  ...
  const fetchAdvertisements = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('http://127.0.0.1:5000/api/advertisements');
  ...
  const handleStatusToggle = async (id) => {
    try {
      ...
      await axios.patch(`http://127.0.0.1:5000/api/advertisements/${id}`, { status: newStatus });
      fetchAdvertisements();
    } catch (error) {
      console.error('Error updating advertisement status:', error);
      setError('Failed to update advertisement status. Please try again.');
    }
  };

  const handleSubmitAdd = async (newAd) => {
    try {
      await axios.post('http://127.0.0.1:5000/api/advertisements', newAd, {
        headers: {
  ...
  const handleSubmitEdit = async (updatedAd) => {
    try {
      const id = updatedAd.get('_id');
      if (!id) {
        throw new Error('Advertisement ID is missing');
      }
      await axios.put(`http://127.0.0.1:5000/api/advertisements/${id}`, updatedAd, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
  ...
  if (isLoading) {
    return <div className="p-4 text-center">Loading advertisements...</div>;
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h1 className="text-xl sm:text-2xl font-semibold text-gray-800">All Advertisements</h1>
        <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-4">
          <div className="relative w-full sm:w-auto">
            <input
              type="text"
      ...
      {advertisements.length === 0 ? (
        ...
        <>
          ...
          <div className="block sm:hidden space-y-4">
            {advertisements.map((ad) => (
              <div key={ad._id} className="bg-white rounded-lg shadow p-4 space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-sm font-medium text-gray-600">Type:</span>
                    <span className="ml-2">{ad.type}</span>
                  ...
                  <button
                    onClick={() => handleStatusToggle(ad._id)}
                    className={`px-3 py-1 rounded-full text-xs font-medium ${ad.status === 'enable'
                        ? 'bg-green-100 text-green-800'
                        : 'bg-red-100 text-red-800'
                      }`}
                  >
                  ...
                  {advertisements.map((ad) => (
                    <tr key={ad._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm">
                        <button
                          onClick={() => handleStatusToggle(ad._id)}
                          className={`px-4 py-1 rounded-full text-sm font-medium ${ad.status === 'enable'
                              ? 'bg-green-100 text-green-800'
                              : 'bg-red-100 text-red-800'
                            }`}
                        >
