...
const testimonials = [
  {
    ...
    image:
      "https://script.viserlab.com/ratelab/assets/images/frontend/testimonial/6354dee02fa141666506464.jpg",
    text: `
    "Hi, I'm Ar Raad, from the USA. I have been using this platform for a while, and it's been an incredible experience. The platform is easy to use, and it has helped me manage my reviews and customer interactions efficiently. It offers great features, and I would highly recommend it to anyone looking for a reliable and user-friendly platform to boost their business."`,
  },
  {
    id: 2,
    name: "Maria Smith",
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto">
        {/* Header */}
        <div className="text-center mb-12 md:mb-16">
          <h3 className="text-[#FFA500] text-lg md:text-xl mb-2 font-semibold tracking-wider">Our User Review</h3>
        ...
        <div className="relative px-4 md:px-12">
          ...
          <button
            onClick={prevTestimonial}
            className="absolute left-4 top-1/2 -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 rounded-full bg-[#8B4513] text-white flex items-center justify-center hover:bg-[#A0522D] transition-all focus:outline-none"
            aria-label="Previous Testimonial"
          >
            <ChevronLeft className="w-5 h-5 md:w-6 md:h-6" />
          </button>
          ...
          <div className="text-center">
            <div className="mb-6">
              <img
                src={testimonials[currentTestimonial].image}
                alt={testimonials[currentTestimonial].name}
                className="w-20 h-20 md:w-24 md:h-24 rounded-full mx-auto border-4 border-[#FFA500] object-cover transition-transform duration-300 ease-in-out transform hover:scale-105"
              />
            </div>
            <h4 className="text-white text-lg md:text-xl font-semibold mb-1">
              {testimonials[currentTestimonial].name}
          ...
          <button
            onClick={nextTestimonial}
            className="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 rounded-full bg-[#8B4513] text-white flex items-center justify-center hover:bg-[#A0522D] transition-all focus:outline-none"
            aria-label="Next Testimonial"
          >
            <ChevronRight className="w-5 h-5 md:w-6 md:h-6" />
          </button>
        ...
        <div className="mt-12 md:mt-20 max-w-3xl mx-auto space-y-4">
          {faqs.map((faq, index) => (
            <div
              key={index}
              className="bg-white bg-opacity-5 rounded-lg overflow-hidden shadow-lg transition-all duration-300"
            >
              <button
                onClick={() => setOpenFaq(openFaq === index ? null : index)}
                className="w-full px-4 py-3 md:px-6 md:py-4 text-left text-white flex justify-between items-center focus:outline-none"
